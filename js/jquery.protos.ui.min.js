...
c.originalData=a.data||[];c.dataChanged=a.dataChanged||f.noop;c.filters=[];c._data=[];c.originalDataLength=c.originalData.length;c.data=[];c.read=function(b,d){var e={};if(d||0===c._data.length)e={page:b||0,itemsPerPage:d};e.filters=[];for(var l in c.filters)e.filters.push(c.filters[l]);var u=new g.deferred;if("function"===typeof a.data.read)return a.data.read(e),u.resolve();f.ajax({type:"json",url:c.originalData.read,contentType:"application/json; charset=utf-8",type:"GET",dataType:"jsonp",data:e,
success:function(a){c.readed(a);u.resolve()},error:function(a){u.reject()}});return u};c.readed=function(a){var e={},f;a.length?(d=a.length,e=a):(d=a.items,e=a.data,f=!0);b(e);c._data=e;return c.dataChanged(f)};var e=function(b){var c=new g.deferred;if("function"===typeof a.data.create)return a.data.create(b),c.resolve();f.ajax({url:a.data.create,data:{items:b}}).done(function(){c.resolve()});return c};c.created=function(a){b(a);c._data=c._data.concat(a);return c.dataChanged()};c.update=function(b){var d=
...
"and"===a?c._data:c.originalData,w;for(w in g){var h=g[w],k;for(k in h)k==f.field&&h[k]==f.value&&b.push(h)}}c._data=c._data.concat(b);b=[]}};c.getOriginalData=function(){return c.originalData};c.set=function(a,b,c){a[b]=c;a.savedChanges=!1;return a};return c};g.draggable=function(a){var b,c,d=f(a.author.selector),e;a.moveParent&&a.isParentDraggable&&(d=f(a.moveParent));e=f(a.moveParent);d.on("mousedown",function(n){function q(a,b,c){var d={};b&&f.extend(d,{left:b+"px"});c&&f.extend(d,{top:c+"px"});
a.css(d)}b=n.clientX-g.getElementOffset(this).left;c=n.clientY-g.getElementOffset(this).top;var m=a.container;if(m)f(p).on("mousemove",function(l){var n=l.clientX-b;l=l.clientY-c;var s=g.getElementOffset(f(m)[0]);m=f(a.container);if(s.left<n&&s.top<l){var h=g.getElementOffset(d[0]).left+d.outerWidth(),k=g.getElementOffset(d[0]).top+d.outerHeight(),p=s.left+m.outerWidth(),s=s.top+m.outerHeight(),r=p-d.outerWidth(),t=s-d.outerHeight()-1;h<=p?q(e,n,""):q(e,r,"");k<s-1?q(e,"",l):q(e,"",t)}});else f(p).on("mousemove",
function(a){var d=a.clientX-b;a=a.clientY-c;g.getElementOffset(f(m)[0]);q(e,d,a)});f(p).on("mouseup",function(){f(p).unbind("mousemove")})});return this};Array.prototype.where=function(a){for(var b=[],c=this.length,d=0;d<c;d++){var e=this[d];!0===a(e)&&b.push(e)}return b};Array.prototype.first=function(a){for(var b=this.length,c=0;c<b;c++){var d=this[c];if(!0===a(d))return d}};Array.prototype.sort=function(a){var b=this.length,c=Math.floor(b/2);if(2>b)return this;for(var d=this.slice(0,c).sort(a),
...
c=0;c<b;c++)a(this[c]);return this};Array.prototype.count=function(){return this.length-1};Array.prototype.skip||(Array.prototype.skip=function(a){if(this instanceof Array)return this.slice(a,this.length)});Array.prototype.take=function(a){if(this instanceof Array)return this.slice(0,a)};g.lazyLoading=function(a){var b={container:f()};a=f.extend(b,a);f(p).scroll(function(){var b=f(window).scrollTop(),d=f(p).height(),e=f(window).height();.95<b/(d-e)&&a.container.trigger("lazyLoad")})};h.listView=function(a){var b=
this,c=a.author[0],d=c.id+"_listItems",e=c.id+"pagerContainer";a=f.extend({width:600,height:400,imageWidth:200,pageSize:15,lazyLoading:!1},a);c.innerHTML=g.generateHTML("ul",[],"",d);f(c)[0].innerHTML+=g.generateHTML("div",[],"",e);b.dataSource=a.data;b.renderPage=function(){a.lazyLoading||f(r+d+" li").remove(".listViewItem");for(var e="",q=b.dataSource.data,m=0;m<q.length;m++)var l=(new g.template(a.templateId,q[m])).render(),e=e+g.generateHTML("li",["listViewItem"],l,"",!1,{"data-uid":q[m].uid});
f(r+d).append(e);f(c).trigger("pageRendered")};b.pager=new g.widgets.pager({pageSize:a.pageSize,dataSource:a.data,container:r+e,pageChanged:b.renderPage,lazyLoading:a.lazyLoading,nextPrevButtons:a.nextPrevButtons});b.pager.changePage(1);return b};h.pager=function(a){function b(){if(n){var b=a.dataSource.items(),c="",d=a.nextPrevButtons,b=Math.ceil(b/a.pageSize);d?c+=g.generateHTML("li",["pageNumber"],"<"):c;for(var e=0;e<b;e++)var q=g.generateHTML("li",["pageNumber"],e+1),c=c+q;d?c+=g.generateHTML("li",
["pageNumber"],">"):c;n[0].innerHTML=g.generateHTML("div",["pager"],g.generateHTML("ul",["pageList"],c));f(function(){f("li").on("click",n,function(){l(this.innerHTML)})})}}var c={pageSize:15,pageChanged:f.noop,nextPrevButtons:!0},d=this,e=1,n=f(a.container),q=a.dataSource;d.currentPage=function(){return parseInt(e)};a=f.extend(c,a);d.changePage=function(b,c){new g.deferred;var d;e=b;d=q.getPageData(b,a.pageSize,c);d instanceof g.deferred?d.done(m):m(d)};d.refresh=function(a){b();d.changePage(e,a)};
q.dataChanged=d.refresh;(function(){a.lazyLoading?(f(n).on("lazyLoad",function(){d.nextPage()}),lazyLoading=new g.lazyLoading({container:n})):b()})();var m=function(b){b[0]&&(f(a.container+" li").each(function(){this.innerHTML==e?f(this).addClass("selectedPage"):f(this).removeClass("selectedPage")}),n.trigger("pageChanged"),a.pageChanged())};d.nextPage=function(){l("&rt;")};d.prevPage=function(){l("&lt;")};var l=function(b){isNaN(parseInt(b))?"&lt;"===b?(e--,1<=e?d.changePage(e):e++):(e++,e<=Math.ceil(q._data.length/
a.pageSize)?d.changePage(e):e--):d.changePage(b)}};g.scrollTo=function(a){var b=a.author;a=a.speed||1E3;f(p.body).animate({scrollTop:g.getElementOffset(b[0]).top},a);return b};g.spa=function(a){var b=this;b.options=f.extend({},a);b.layout=b.options.author;b.routes=b.options.routes;b.startRouting=function(){c(window.location.hash);if("onhashchange"in window)window.onhashchange=function(){c(window.location.hash)};else{var a=window.location.hash;window.setInterval(function(){window.location.hash!=a&&
(a=window.location.hash,c(a))},100)}};b.stopRouting=function(){clearInterval(!0);window.onhashchange=f.noop};b.navigate=function(a){c("#"+a)};var c=function(a){for(var c in b.routes){var f=b.routes[c].route,m;1===a.indexOf("/")&&0===f.indexOf("/")?m=!1:m=!0;var l;l="#"+f;var h=a,k=m;if(l===h)l=!0;else{var p=l.match(RegExp(":[a-zA-Z0-9.\\-~_+%]+","g")),r=void 0;if(null===p)l=!1;else{r=k?g.jsonToArray(g.queryStringToJson(h)):g.routeToArray(h);k=void 0;for(k in p)l=l.replace(p[k],r[k]);l=l===h}}if(l){c=
b.routes[c].action;if("undefined"!==typeof c&&"function"===typeof c)return a=m?g.queryStringToJson(a):g.routeToArray(a),c(a||{});if("undefined"===typeof c)return d(f);if("string"===typeof c)return d(c);break}}},d=function(a){f.ajax({url:a,type:"html",method:"get"}).done(function(a){b.layout.html(a)})};return b};h.shake=function(a){var b=this,c=a.author,d=a.speed,e=-1*a.distance,f=a.vertical,g,h;c.on(a.event,function(){b.start()});b.start=function(){c.trigger("shakingStarts");g=setInterval(function(){c.css({"-webkit-transform":"translate("+
...
1;if(0>e)return!1;c+=d;if(c>e)return c=a.repeatImages?0:e,!1;if(0>c)return c=a.repeatImages?e:0,!1},k=b.popUp.show,m=b.popUp.hide;b.popUp.show=function(){k();e(b.images[0]);f("#nextImg").on("click",function(){b.nextImage()});f("#prevImg").on("click",function(){b.previousImage()});f("body").on("keydown",function(a){37==a.keyCode&&b.previousImage();39==a.keyCode&&b.nextImage()})};b.popUp.hide=function(){m();f("#nextImg").off("click");f("#prevImg").off("click");f("body").off("keydown")};b.removeImages=
...
f.extend({width:500,height:300,darkness:.3,title:"Window",draggable:!0},a);return new h.popUpCore(a)}})(jQuery,document,"#");